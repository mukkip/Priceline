...
public class FourPlayers {
	...
	//Creating Chutes and Ladders classes
	...
	void gameStart() {
		//Creating profile to all the players.
		p1.setName("Eric");
		p1.setNloc(0);
		p1.setLoc(0);
		p2.setName("Paul");
		...
		while (p1.getNloc() <= 100 && p2.getNloc() <= 100) {
			...
			//Player 3 Turn
			 temp = p3.getNloc();
			p3.setNloc(p3.getNloc() + randNumber());
			if (p3.getNloc() > 100) {
				p3.setNloc(temp);
				p3.setLoc(temp);
			} 
			else if (p3.getNloc() == 100) {
				System.out.println(" The Winner is " + p3.getName());
				break;
			} 
			else {
				p3.setLoc(temp);
			...
			count++;
			ccheck = c.chutes(p1.getNloc());
			lcheck = l.ladder(p1.getNloc());
			if (ccheck == 0) {

				if (lcheck == 0) {
					System.out.println(count + "." + p3.getName() + ": " + p3.getLoc() + " --> " + p3.getNloc());
				} 
				else if (lcheck==100) {
					System.out.println(count + "." + p3.getName() + ": " + p3.getLoc() + " --> " + p3.getNloc()
					+ " --LADDER-->" + lcheck);
			...
			/* Player 4 Turn */
			temp = p4.getNloc();
			p4.setNloc(p4.getNloc() + randNumber());
			if (p4.getNloc() > 100) {
				p4.setNloc(temp);
				p4.setLoc(temp);

			}
			else if (p4.getNloc() == 100) {

				System.out.println("The Winner is " + p4.getName());
				break;
			} else {
				p4.setLoc(temp);
			}
			...
			ccheck = c.chutes(p4.getNloc());
			lcheck = l.ladder(p4.getNloc());

			if (ccheck == 0) {

				if (lcheck == 0) {
					System.out.println(count + "." + p4.getName() + ": " + p4.getLoc() + " --> " + p4.getNloc());
				} 
				...
				System.out.println(count + "." + p4.getName() + ": " + p4.getLoc() + " --> " + p4.getNloc()	+ " --CHUTE-->" + ccheck);
				p4.setNloc(ccheck);
				p4.setLoc(ccheck);
			}
